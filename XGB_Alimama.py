...
def extract_frist_time(train, test):
    ...
    # 用户当天点击了多少对应价格的
    x = train.groupby(['user_id', 'item_sales_level', 'day']).size().reset_index().rename(columns={0: 'user_day_page'})
...
def getFeature():
    ...
    extract_feature = ['occup_rate', 'occup_cate_rate', 'occup_city_rate', 'occup_brand_rate', 'occup_hour_rate',
                       ...
                       'age_hour_rate', 'is_last', 'user_item_sum', 'age_page_rate', 'occip_gender_page_rate',
                       'occup_page_rate', 'star_price_rate',
...
def split_feature_train_test(data, online):
    if online == False:
        ...
        test = pd.read_csv(r'~/Alimama/round2_ijcai_18_test_a_20180425.txt', sep=' ')
...
def xgboost_model(train, test, features, target):
    ...
    use_features = [
        'is_trade', 'context_page_id', 'item_id', 'item_brand_id', 'item_city_id', 'user_gender_id','time'
        ...
        'shop_mean_price', 'mean_brand_prices', 'low_number', 'occup_rate', 'occup_cate_rate', 'max_occup',
    ...
    test_features = [
        'context_page_id', 'item_id', 'item_brand_id', 'item_city_id', 'user_gender_id','time'
        'shop_id', 'user_id', 'category_0', 'category_1', 'category_2', 'property_0', 'mean_price_age',
        'property_2', 'predict_category_0', 'predict_category_1', 'predict_category_2', 'item_price_level',
        'item_sales_level', 'user_star_level', 'item_collected_level', 'item_pv_level', 'user_age_level',
        'shop_review_num_level', 'shop_star_level', 'shop_review_positive_rate', 'shop_score_service',
        'shop_score_delivery', 'shop_score_description', 'hour', 'mean_item_prices', 'mean_brand_age',
        'shop_mean_price', 'mean_brand_prices', 'low_number', 'occup_rate', 'occup_cate_rate',
        'occup_city_rate', 'occup_brand_rate', 'occup_hour_rate', 'age_cate_rate', 'age_brand_rate',
        'item_id_rank', 'item_id_sum', 'shop_id_rank', 'shop_id_sum', 'age_hour_rate', 'is_last', 'property_1',
        'user_item_sum', 'age_page_rate', 'occip_gender_page_rate', 'occup_page_rate', 'star_price_rate',
        'page_rate', 'pv_rate', 'sales_level_rate', 'user_day_cate_1', 'user_shop_day', 'dayclick_times',
    ...
    clf = xgb.train(params, dtrain, num_boost_round=2000, verbose_eval=10)
...
def abnormal_processing(data):

    abnormal_features = ['item_price_level', 'item_sales_level', 'user_star_level',
                         'item_collected_level', 'item_pv_level', 'user_age_level',
                         'shop_review_num_level', 'shop_star_level', 'shop_review_positive_rate',
                         'shop_score_service', 'shop_score_delivery', 'shop_score_description', 'low_number']

    for i in range(len(abnormal_features)):
        mean = np.mean(data[abnormal_features[i]])
        std = np.std(data[abnormal_features[i]])
        pyplot.plot(data[abnormal_features[i]], 'r--')

        for j in range(len(data[abnormal_features[i]])):
            if data[abnormal_features[i]].values[j] > mean + 3*std:
                data[abnormal_features[i]].values[j] = mean + 3*std
            elif data[abnormal_features[i]].values[j] < mean-3*std:
                data[abnormal_features[i]].values[j] = mean - 3*std

        pyplot.plot(data[abnormal_features[i]])
...
if __name__ == "__main__":
    ...
    data = pd.read_csv(r'~/Alimama/round2_train.txt', sep=' ')
    ...
    test = test.fillna(1)
    # train = abnormal_processing(train)
    ...
    pd.DataFrame(train.append(test)).to_csv('new_train.csv')
    ...
    if not online:
        ...
        test[['instance_id', 'predicted_score']].to_csv('414.txt', index=False, sep=' ')  # 保存在线提交结果
    print("输出完成")
